...
headers = {'user-agent': f'Tianyin class practice(qhh3bv@virginia.edu) (Language={sys.version})'} # example header based on AWS


def clean_text(text):
    text = re.sub(r'(?i)\b(?:ap|p)\.\s*\d+', '', text)
...
index = iching.find_all('a', href = lambda x: x and x.startswith('ic')) # left out intro and plates
...
tr2 = re.sub(r'</?[Ii]>', '', tr.text) # getting rid of the italic tags
...
content = " ".join(p.get_text() for p in plist if p.get_text())
...
df["content"] = (
    df["content"]
    .str.replace(r"(?i)\b(?:ap|p)\.\s*\d+", "", regex=True)
